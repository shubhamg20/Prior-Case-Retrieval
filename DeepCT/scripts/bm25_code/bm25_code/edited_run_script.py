...
save_folder = f'./ucreate_results/EXP_{current_time}'
...
config_dict={
    ...
    'query_weights_path' : '/workspace/tejas/PCR/DeepCT/scripts/comb_query_weights_9_epoch.json'
}
...
#                                           ngram_range=(2, 2))
    ...
    def transform(self, q, X, term_wdict):
        ...
        for idx in q.indices:
            query_term=feature_names[idx]
            if str(query_term) in term_wdict.keys():
                query_term_weight.append(term_wdict[str(query_term)])
            else:
                counter=counter+1
                query_term_weight.append(0)
