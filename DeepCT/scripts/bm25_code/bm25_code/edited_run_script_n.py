...
save_folder = f'./filter_1_6&2_7/EXP_{current_time}'
...
#                                           ngram_range=(2, 2))
    ...
    def transform(self, q, X, term_wdict):
        ...
        weight_max = max(query_term_weight)
        query_term_weight = query_term_weight/weight_max
        
        e_term_weight = query_term_weight/(abs(1-query_term_weight)+(0.00001))
        
        denom = X + (k1 * (1 - b + b * len_X / avdl))[:, None]
        # idf(t) = log [ n / df(t) ] + 1 in sklearn, so it need to be coneverted
        # to idf(t) = log [ n / df(t) ] with minus 1
        idf = self.vectorizer._tfidf.idf_[None, q.indices] - 1.
        idf = np.log(np.multiply(np.exp(idf),e_term_weight.reshape(idf.shape[0],idf.shape[1]))) 
...
# python get_deepct_weights.py /workspace/tejas/PCR/DeepCT/data/PCR_UCREAT/test.tsv /workspace/tejas/PCR/DeepCT/collection_9_epoch/test_results.tsv query_weights.json 1 --output_format=json

# python comb_query_weights.py 
